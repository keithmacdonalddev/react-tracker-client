...
export const showWidgetItemReducer = (
	state = { component: 'tickets' },
	action,
) => {
	switch (action.type) {
...
export const usersNavActiveReducer = (
	state = { usersNavState: 'Friends' },
	action,
) => {
	switch (action.type) {
		case USERS_NAV_ACTIVE:
			return { usersNavState: action.payload };
		default:
			return state;
	}
};

...
import { createAlert, modalStatusAction, showComponent } from 'store/actions/navigationActions';
...
export const Form = () => {
	...
	const { loadingCreate, errorCreate, successCreate } = useSelector((state) => state.ticketCreate);
	...
	const submitHandler = (event) => {
		...
		dispatch(createTicket(title, description, status, developer, priority, project, userInfo));
	...
	return (
		<div className={open ? style.createTicketContainerModal : style.createTicketContainer}>
				...
				<div className={style.formGrid}>
					...
					<div className={style.dataColumn}>
						...
						{loading ? (
							...
							<select required className={style.developerSelect} onChange={(event) => setDeveloper(event.target.value)}>
								<option></option>
								{users.map((developer) => (
									<option key={developer.name}>{developer.name}</option>
								))}
							</select>
						) : (
						...
						<input className={style.submittedByInput} type='text' value={userInfo.name} readOnly />
